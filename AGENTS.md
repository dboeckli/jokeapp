...
Jokeapp is a Spring Boot app (**Java 25**) that generates random Chuck Norris jokes (via the
`ChuckNorrisQuotes` library) and serves them through a simple web UI (port `8080`). Single Maven
module, package `guru.springframework.joke.jokeapp`.

## Build & test commands

- Full build: `./mvnw clean verify` — spring-javaformat + Spotless format checks, unit tests
  (`*Test`, surefire) and integration tests (`*IT`, failsafe), Helm lint/template.
- Unit tests only: `./mvnw test`. Single test: `./mvnw test -Dtest=JokeControllerTest#methodName`.
...
- Helm-only: chart in `helm-charts/`, packaged to `target/helm/repo/`, release name = artifactId,
  namespace `jokeapp`, NodePort `30080`.
